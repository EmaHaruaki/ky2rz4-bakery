<template>
	<div id="not_top">
		...
		<div>
			<template v-for="n in response_category.list">
				<div :key="n.topics_category_id" v-if="n.topics_category_id == $route.params.slug"
					class="archive_title">
					{{ n.category_nm }}</div>
			</template>
		</div>
			...
			<nav class="navigation pagination" role="navigation">
				...
				<div class="nav-links">
					<nuxt-link v-if="response_list.pageInfo.pageNo != response_list.pageInfo.startPageNo"
						class="prev page-numbers" :to="`/blog/${response_list.pageInfo.pageNo - 1}`">&laquo; 前へ
					</nuxt-link>
					<nuxt-link v-for="i in response_list.pageInfo.totalPageCnt" :key="i" class='page-numbers'
						:to="`/blog/${i}`" :class="{ 'current': response_list.pageInfo.pageNo === i, }">{{ i }}
					</nuxt-link>
					<nuxt-link v-if="response_list.pageInfo.pageNo != response_list.pageInfo.endPageNo"
						class="next page-numbers" :to="`/blog/${response_list.pageInfo.pageNo + 1}`">次へ &raquo;
					</nuxt-link>
				</div>
			</nav>
		</div>
...
export default {
	async asyncData({ $axios, params }) {
		return {
			response_list: await $axios.$get('/rcms-api/1/blog/list', { params: { contents_type: params.slug } }),
